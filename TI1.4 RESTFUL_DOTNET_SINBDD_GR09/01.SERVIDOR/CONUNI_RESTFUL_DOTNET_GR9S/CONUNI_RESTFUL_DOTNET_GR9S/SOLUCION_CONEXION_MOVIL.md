...
2. **Agregar binding HTTP:**
   ...
   ```xml
   <bindings>
       <binding protocol="http" bindingInformation="*:44385:127.0.0.1" />
...
3. **Configurar URL ACL (PowerShell como Administrador):**
   ```powershell
   netsh http add urlacl url=http://127.0.0.1:44385/ user=Everyone
...
```
http://127.0.0.1:44385/api/conversion/info
```
...
<site name="CONUNI_RESTFUL_DOTNET_GR9S" id="1">
    <bindings>
        ...
        <binding protocol="https" bindingInformation="*:44384:127.0.0.1" />
...
```powershell
# Como Administrador
netsh http add urlacl url=https://127.0.0.1:44384/ user=Everyone
...
3. En **Bindings**, agrega:
   - **Type**: http
   - **IP Address**: 127.0.0.1 (o All Unassigned)
   - **Port**: 44385
...
```powershell
# HTTP
Invoke-WebRequest -Uri "http://127.0.0.1:44385/api/conversion/info"

# HTTPS
Invoke-WebRequest -Uri "https://127.0.0.1:44384/api/conversion/info" -SkipCertificateCheck
```
...
3. **Verifica la IP:**
   ...
   ipconfig
   ```
   Asegúrate de usar la IP correcta (127.0.0.1)
